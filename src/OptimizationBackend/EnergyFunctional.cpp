...
double EnergyFunctional::calcMEnergyF()
{
	...
	VecX delta = getStitchedDeltaF();//各变量的差
...
double EnergyFunctional::calcLEnergyF_MT()
{
	...
	double E = 0;
	for(EFFrame* f : frames)
        E += f->delta_prior.cwiseProduct(f->prior).dot(f->delta_prior);//状态变量的差的平方
	...
	for(EFFrame* f : frames)
	{
		for(int i=0;i<(int)f->points.size();i++)
		{
			...
			if(p->stateFlag == EFPointStatus::PS_MARGINALIZE)
			{
				...
				for(EFResidual* r : p->residualsAll)
					if(r->isActive())
					  connectivityMap[(((uint64_t)r->host->frameID) << 32) + ((uint64_t)r->target->frameID)][1]++;//用于显示？
...
{// 这里面的ｌａｍｄｂａ起到了什么作用
	...
	if(setting_solverMode & SOLVER_SVD)
	{
		...
		x = SVecI.asDiagonal() * svd.matrixV() * Ub;//解出x来
...
{//把各个帧的ID重新整理一下
	for(unsigned int idx=0;idx<frames.size();idx++)//参与误差计算的帧
		frames[idx]->idx = idx;//按顺序给ID号

	allPoints.clear();//把点都清楚,allPoint参与物产计算的点

	for(EFFrame* f : frames)
		for(EFPoint* p : f->points)
		{
			allPoints.push_back(p);//将参与误差计算的帧对应的点赋值给allPoint
			for(EFResidual* r : p->residualsAll)
			{
