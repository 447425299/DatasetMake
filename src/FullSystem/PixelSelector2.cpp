...
{
	gradHistFrame = fh;//每一帧，６层金字塔只处理这一次
	float * mapmax0 = fh->absSquaredGrad[0];//指针指向的是数组  梯度的平方和
	...
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			...
			memset(hist0,0,sizeof(int)*50);//都设为０，gradHist维度１００＊(1+w/32)*(1+h/32),把前１００设为0  把直方图分成５０个级别

			for(int j=0;j<32;j++) for(int i=0;i<32;i++)
			{
				int it = i+32*x;
				int jt = j+32*y;
				if(it>w-2 || jt>h-2 || it<1 || jt<1) continue;//边上的不要
			...
			ths[x+y*w32] = computeHistQuantil(hist0,setting_minGradHistCut) + setting_minGradHistAdd;//找了一个中值，梯度中占中间的
		}
	...
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			...
			thsSmoothed[x+y*w32] = (sum/num) * (sum/num);//做一个平滑？
...
//	else
	{
		...
		// K / (pot+1)^2, where K is a scene-dependent constant.  //存在的越多选的越少。
		// we will allow sub-selecting pixels by up to a quotia of 0.25, otherwise we will re-select.

		if(fh != gradHistFrame) makeHists(fh); //意思就是make过之后就不make了

		// select!
		Eigen::Vector3i n = this->select(fh, map_out,currentPotential, thFactor);

		// sub-select!
		numHave = n[0]+n[1]+n[2];//3层中选出的点
		quotia = numWant / numHave;
