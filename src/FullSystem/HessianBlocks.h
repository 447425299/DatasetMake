...
struct FrameHessian
{
	...
	Eigen::Vector3f* dI;//分别代表 像素值,dx,dy	根据图像求出			 // trace, fine tracking. Used for direction select (not for gradient histograms etc.)
	...
	int frameID;//关键帧的ID		// incremental ID for keyframes only!
	static int instanceCounter;
	int idx;//所有帧的ID

	// Photometric Calibration Stuff 光度矫正的相关变量
	float frameEnergyTH;	// set dynamically depending on tracking residual THThreshold为阈值的意思
	float ab_exposure;//曝光时间,提前保存好的

	bool flaggedForMarginalization;

	std::vector<PointHessian*> pointHessians;				// contains all ACTIVE points.
	std::vector<PointHessian*> pointHessiansMarginalized;	// contains all MARGINALIZED points (= fully marginalized, usually because point went OOB.)// OOB到底什么意思?
	...
	void setStateZero(const Vec10 &state_zero);//设置零空间,求解帧和地图点旋转所需要的矩阵
	...
	inline void setEvalPT_scaled(const SE3 &worldToCam_evalPT, const AffLight &aff_g2l)
	{
		...
		setStateZero(this->get_state());//清零
	};
...
struct CalibHessian
{
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
	static int instanceCounter;
	//和相机几何参数相关的量 fx fy cx cy
	VecC value_zero;
	VecC value_scaled;
	VecCf value_scaledf;
	VecCf value_scaledi;
	VecC value;
	VecC step;//高斯牛顿求出来的增量
	VecC step_backup;
	VecC value_backup;
...
struct PointHessian
{
	...
	// static values
	bool color16_bool;//超出边界的点就不要了
	float color[MAX_RES_PER_POINT];			// colors in host frame
	float color16[patternPatchNum];			// colors in host frame
	...
	float idepth_zero;
	float idepth;
	float step;//这是什么意思?步长?求雅克比矩阵用的吧
	float step_backup;
	float idepth_backup;
	...
	void makepatch(FrameHessian* frame, const Mat33f &K, const SE3 &hostToNew, const Vec2f &hostToFrame_affine, CalibHessian* HCalib, bool debugPrint=false);
	...
	std::pair<PointFrameResidual*, ResState> lastResiduals[2]; 	//和前两个帧的误差// contains information about residuals to the last two (!) frames. ([0] = latest, [1] = the one before).
