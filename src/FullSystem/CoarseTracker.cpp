...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians)
{
	...
	memset(idepth[0], 0, sizeof(float)*w[0]*h[0]);//都给赋值零
	...
	{//遍历优化窗口中的每一个帧
		...
		{//遍历该帧对应的地图点
			...
			{//对于计算出误差的正常投影点
				...
				int u = r->centerProjectedTo[0] + 0.5f;//投影点位置
...
{//Vec6 resOld = calcRes(lvl, refToNew_current, aff_g2l_current, setting_coarseCutoffTH*levelCutoffRepeat);//setting_coarseCutoffTH==20 阈值参数

	float E = 0;
	int numTermsInE = 0;//加入误差计算的像素数目
	int numTermsInWarped = 0;
	int numSaturated=0;//超过最大值的数目
...
//转换矩阵
	...
	for(int i=0;i<nl;i++)
	{
		...
		if(lvl==0 && i%32==0)// 这是什么意思?? 32从何而来,每32像素中必有一个,也许就是做一个大致估计,32个限速估计一次
		{
		...
		if(!(Ku > 2 && Kv > 2 && Ku < wl-3 && Kv < hl-3 && new_idepth > 0)) continue;//超出便捷就算了



		float refColor = lpc_color[i];
        Vec3f hitColor = getInterpolatedElement33(dINewl, Ku, Kv, wl);// NOTE 并没有解决远近的问题,而是各层和各层比较
		...
		{//
			...
			numTermsInWarped++;//未超出阈值的像素数目
		}
	}

	while(numTermsInWarped%4!=0)// SSE计算的时候用得着   每隔4个赋值个领是什么意思?
	{
	...
	buf_warped_n = numTermsInWarped;//SSE计算的时候用得着
	...
	Vec6 rs;
	rs[0] = E;//误差
	rs[1] = numTermsInE;//参与误差计算的像素个数
	rs[2] = sumSquaredShiftT/(sumSquaredShiftNum+0.1);//一正,一负的移动造成的变化
	rs[3] = 0;
	rs[4] = sumSquaredShiftRT/(sumSquaredShiftNum+0.1);
	rs[5] = numSaturated / (float)numTermsInE;//超出限制值的比例
...
{//
	...
	makeCoarseDepthL0(frameHessians);//参考帧的深度和权重


	refFrameID = lastRef->shell->id;
	lastRef_aff_g2l = lastRef->aff_g2l();

	firstCoarseRMSE=-1;//这里给了-1 但是从来没有用过呀
		...
		IOWrap::Output3DWrapper* wrap)
{
	...
	assert(coarsestLvl < 5 && coarsestLvl < pyrLevelsUsed);//维度是5 如果后面改了金字塔的层数要提醒
	//先给一个最大值再说
	...
	int maxIterations[] = {10,20,50,50,50};//每一层迭代的次数
	float lambdaExtrapolationLimit = 0.001;// 什么意思?
//给初始值
	...
	{//一层层的来
		Mat88 H; Vec8 b;//求解方针用的矩阵和向量  传统的8点法??
		float levelCutoffRepeat=1;
		Vec6 resOld = calcRes(lvl, refToNew_current, aff_g2l_current, setting_coarseCutoffTH*levelCutoffRepeat);//setting_coarseCutoffTH==20 阈值参数
		while(resOld[5] > 0.6 && levelCutoffRepeat < 50)
		{//超出阈值的比例大了就增大阈值(2倍)再次计算,但是不能超过50倍
			levelCutoffRepeat*=2;
		...
		//这个矩阵求出来就不变了,节省时间,效果也不错  最终还是没有把SSE怎么用的整明白
		calcGSSSE(lvl, H, b, refToNew_current, aff_g2l_current);//SSE求解 求出H和B来 
		...
		//迭代求解  求解过程 NOTE 没看这部分
		for(int iteration=0; iteration < maxIterations[lvl]; iteration++)
		{
			Mat88 Hl = H;
			for(int i=0;i<8;i++) Hl(i,i) *= (1+lambda);//lamda各有各的用处
			...
			aff_g2l_new.b += incScaled[7];//更新变量
		...
		if(lastResiduals[lvl] > 1.5*minResForAbort[lvl]) return false;//比最优的差的多,就不再迭代了
