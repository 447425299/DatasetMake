...
void FullSystem::linearizeAll_Reductor(bool fixLinearization, std::vector<PointFrameResidual*>* toRemove, int min, int max, Vec10* stats, int tid)
{
	for(int k=min;k<max;k++)
	{
		...
		(*stats)[0] += r->linearize(&Hcalib);//返回广度误差 并线性化了雅克比矩阵

		if(fixLinearization)
		{
			r->applyRes(true);//更新状态和误差量,
...
void FullSystem::setNewFrameEnergyTH()
{
	...
	for(PointFrameResidual* r : activeResiduals)
		if(r->state_NewEnergyWithOutlier >= 0 && r->target == newFrame)//target是当前帧
		{
	...
	if(allResVec.size()==0)
	{
		newFrame->frameEnergyTH = 12*12*patternNum;//这是最大的了
	...
	assert(nthIdx < (int)allResVec.size());//这俩不一回事嘛;
	assert(setting_frameEnergyTHN < 1);

	std::nth_element(allResVec.begin(), allResVec.begin()+nthIdx, allResVec.end());//[ )左闭又开的区间
	float nthElement = sqrtf(allResVec[nthIdx]);//排序,这要这个位置的排队了就行
...
Vec3 FullSystem::linearizeAll(bool fixLinearization)
{
	...
	else
	{
		...
		lastEnergyP = stats[0];//总的光度误差
	}


	setNewFrameEnergyTH();//根据光度误差给个阈值
...
//	meanStepC += Hcalib.step.norm();
	...
	EFDeltaValid=false;
	setPrecalcValues();//更新相机内参,及相互之前的关系,这些是需要提前算的那些变量.
...
{
	if(setting_solverMode & SOLVER_MOMENTUM)// 这样又有什么意义呢?
	{
		if(backupLastStep)
		{
			Hcalib.step_backup = Hcalib.step;//相机内参
			Hcalib.value_backup = Hcalib.value;//相机内参
			for(FrameHessian* fh : frameHessians)
			{
...
{//开始优化啦
	...
	for(FrameHessian* fh : frameHessians)
		for(PointHessian* ph : fh->pointHessians)
		{
			...
			{
				if(!r->efResidual->isLinearized)// 没有被抛弃的点就是已经线性化过的.
				{//不参与优化的点?这些点比较好,不用了
					activeResiduals.push_back(r);// 这里没有线性化的点为active??
					r->resetOOB();
				}
				else
					numLRes++;
	...
	double lastEnergyL = calcLEnergy();//不用算
	double lastEnergyM = calcMEnergy();//不用算

	//将偏导数应用到各个点上
	if(multiThreading)// 这里reduce 怎么用的?bind 这个计算量也不大啊,为什么非要用多线程
	...
	double lambda = 1e-1;
	float stepsize=1;
	VecX previousX = VecX::Constant(CPARS+ 8*frameHessians.size(), NAN);//要优化的参数
	for(int iteration=0;iteration<mnumOptIts;iteration++)//NOTE 开始迭代 TODO 迭代的过程不处理点
	{
		// solve!
		backupState(iteration!=0);// 把上一步的值保存了再说,保存步长,为了牛顿迭代
		//solveSystemNew(0);
		solveSystem(iteration, lambda);//求解的过程,这里应该是状态变量的增量吧
		double incDirChange = (1e-20 + previousX.dot(ef->lastX)) / (1e-20 + previousX.norm() * ef->lastX.norm());//Dir direction 方向 inc increase
		previousX = ef->lastX;//最后的X,最后解出来的X


		if(std::isfinite(incDirChange) && (setting_solverMode & SOLVER_STEPMOMENTUM))
		{
			float newStepsize = exp(incDirChange*1.4);//步长
			if(incDirChange<0 && stepsize>1) stepsize=1;//意思就是方向上差的大了就

			stepsize = sqrtf(sqrtf(newStepsize*stepsize*stepsize*stepsize));//确定迭代步长
			if(stepsize > 2) stepsize=2;
			if(stepsize <0.25) stepsize=0.25;
		}
		//之前解出来的是增量,现在把原来的加上,就更新为新的状态变量
		...
		// eval new energy!
		Vec3 newEnergy = linearizeAll(false);//0.总的光度误差  为什么用false,不是求一次就行
		double newEnergyL = calcLEnergy();//各优化变量变化的平方乘以上一步的值
		double newEnergyM = calcMEnergy();// delta.dot(2*bM + HM*delta);新的状态变量得到的值
		...
		if(setting_forceAceptStep || (newEnergy[0] +  newEnergy[1] +  newEnergyL + newEnergyM <
				lastEnergy[0] + lastEnergy[1] + lastEnergyL + lastEnergyM))//强制执行
		{

			if(multiThreading)// 这里的多线程是怎么执行的 reduce()  _1, _2, _3, _4 各有什么作用 按4个线程执行?这个地方在哪定义的也没找到?
				treadReduce.reduce(boost::bind(&FullSystem::applyRes_Reductor, this, true, _1, _2, _3, _4), 0, activeResiduals.size(), 50);//同时执行
			else
				applyRes_Reductor(true,0,activeResiduals.size(),0,0);
	...
	lastEnergy = linearizeAll(true);// 这是为什么啊?不是已经算过了吗?
