...
public:
	...
	void print_So_Cost(std::string file);
	void print_Sp(std::string file);

	void printTimePerFrame(const std::string &filename,std::vector<float> &vTimesTrackToPaint);
	...
	bool initFailed;
	bool initialized;
	bool linearizeOperation;//程序运行速度,preset==0的时候执行该操作设置为1 ,为2000点,且非实时的时候(越快越好)
...
private:
	...
	CalibHessian Hcalib;//图像本身的相关参数,及其矫正参数
	...
	Vec4 trackNewCoarse(FrameHessian* fh);//跟踪确定位姿
	void traceNewCoarse(FrameHessian* fh);//完成匹配,在当前帧中找到与目标点匹配的像素. 少朋
	void activatePoints();
	void activatePointsMT();
	void activatePointsOldFirst();
	void flagPointsForRemoval();
	void makeNewTraces(FrameHessian* newFrame, float* gtDepth);//生成新的被跟踪的点
	...
	Vec3 linearizeAll(bool fixLinearization);// ture 和False 区别在于是否更新状态变量?
	...
	std::vector<VecX> getNullspaces(//求解零空间
	...
	boost::mutex trackMutex;//就在跟踪的线程里加锁了,也是醉了
	...
	std::vector<FrameHessian*> frameHessians;//参与优化的关键帧	// ONLY changed in marginalizeFrame and addFrame.
	...
	boost::mutex coarseTrackerSwapMutex;	//用ＩＤ的时候定义的互斥体		// if tracker sees that there is a new reference, tracker locks [coarseTrackerSwapMutex] and swaps the two.
 ...
 */
	...
	boost::thread mappingThread;//见图线程的初始化
	bool runMapping;
