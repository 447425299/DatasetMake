		...
		bool plot)
{
	...
	for(int i=0;i<npts;i++)
	{
		...
		for(int idx=0;idx<patternNum;idx++)
		{
			...
			if(!(point->u+dx > 1&&  point->v+dy > 1 && point->u+dx < wl-2 &&  point->v+dy < hl-2))//change8 尝试减去约束
			{
				dx = 0;
				dy = 0;
			}
...
// 			if(idx%100==0)
// 			{
// 			  std::cout<<"rlR "<< point->u+dx<<"  "<<point->v+dy<<std::endl;
// 			}
...
void CoarseInitializer::setFirst(	CalibHessian* HCalib, FrameHessian* newFrameHessian)//读过了
{

	makeK(HCalib);//得到相机内参　
	firstFrame = newFrameHessian;

	PixelSelector sel(w[0],h[0]);//金字塔最底层
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		Pnt* pl = points[lvl];
		int nl = 0;
		for(int y=patternPadding+1;y<hl-patternPadding-2;y++)//change8
